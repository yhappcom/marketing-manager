...
Import has four verified activation concerns: discoverability asymmetry, post-save value-handoff gap, non-USD required-field workload, and currency-semantic risk. Import's save→first-value handoff is B3 and ad-ineligible. Background/foreground transitions do not reset that protection under 070.
...
6. Build the first ad-placement ledger from actual product surfaces using 069. For MintTap, verify the complete Home value block before the current inline ad and obtain placement-level aggregate request/impression/revenue evidence.
